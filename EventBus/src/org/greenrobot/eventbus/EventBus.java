...
/**
 * EventBus是为安卓设计的中央订阅发布系统，事件被post进Bus里面，然后被传送到订阅者中能匹配事件类型的方法中，为了能够
 * 收到事件，订阅者必须通过register方法把自己注册给Bus，一旦注册，订阅者就能接受到事件，除非调用了unRegister取消了注册。
 * 订阅者的事件处理方法必须用Subscribe注解标注，必须是public类型，无返回值，只能有一个参数
 *
 * -----------------------------------------------------------------------------------------------------
 * , which delivers it to subscribers that have a matching handler method for the event type. To receive events,
...
public class EventBus {
    ...
    /**
     * 把当前类注册成订阅者，不再关心该事件的时候记得取消注册
     * 该订阅类必须有一个被Subscribe注解标注的事件处理方法，该注解允许配置ThreadMode
     * ------------------------------------------------------------
     *
    ...
    private void checkPostStickyEventToSubscription(Subscription newSubscription, Object stickyEvent) {
        if (stickyEvent != null) {
            //订阅者不能试图打断事件，因为事件在posting状态的时候不能被跟踪
            // If the subscriber is trying to abort the event, it will fail (event is not tracked（跟踪） in posting state)
    ...
    private void postSingleEvent(Object event, PostingThreadState postingState) throws Error {
        ...
        if (eventInheritance) {/**考虑事件的继承性，extends||implements*/
            List<Class<?>> eventTypes = lookupAllEventTypes(eventClass);
            int countTypes = eventTypes.size();
            //循环发送事件
            for (int h = 0; h < countTypes; h++) {
